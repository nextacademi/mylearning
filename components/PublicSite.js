...
import { loadPublicStats } from "../lib/public-stats-data";
import PeopleDotCloud from "./public/PeopleDotCloud";
...
const galleryItems = [
  { src: "/tranning2.jpeg", category: "Education", title: "Classroom Sessions" },
  { src: "/tranning18.jpeg", category: "Education", title: "Hands-on Training" },
  { src: "/tranning145.jpeg", category: "Community", title: "Group Activities" },
  { src: "/tranning195.jpeg", category: "Community", title: "Team Building" },
  { src: "/WhatsApp Image 2026-09-09 at 11.15.10 PM.jpeg", category: "Highlights", title: "Recent Moments" },
];
...
const HOW_TO_HELP = [
  {
    ...
    description: "Enroll in a live batch and learn alongside people building the same skills you are.",
  },
...
const TESTIMONIALS = [
  {
    ...
    quote: "The sessions gave me language for things I had felt but could not yet explain.",
  },
  {
    avatar: photos.mentor,
    name: "Dara K.",
    role: "Teacher participant",
    quote: "I left with tools I could use with my students the very next morning.",
  },
  {
...
function Eyebrow({ children, className = "", light = false }) {
  return (
    ...
    >
      <span className={`h-[2px] w-6 ${light ? "bg-[#FCA5A5]" : "bg-[#E53935]"}`} />
      {children}
    </p>
  );
}
...
function AnimatedCounter({ value, suffix = "", active, duration = 1800 }) {
  ...
  return (
    <span className="tabular-nums inline-block" style={{ minWidth: `${String(value).length + suffix.length}ch` }}>
      {display}
      {suffix}
    </span>
  );
...
function HeroStatsRow({ items }) {
  ...
  return (
    <div ref={ref} className="mt-10 grid grid-cols-2 gap-x-6 gap-y-6 border-t border-white/10 pt-8 sm:grid-cols-4">
      {items.map((stat, index) => (
        <div key={stat.label} className={`${index !== 0 ? "sm:border-l sm:border-white/10 sm:pl-6" : ""}`}>
          <strong className="block text-3xl font-black text-white">
            <AnimatedCounter value={stat.value} suffix={stat.suffix} active={visible} />
          </strong>
          <span className="mt-1 block text-xs font-medium text-white/50">{stat.label}</span>
        </div>
      ))}
    </div>
  );
...
export default function PublicSite() {
  ...
  // Real hero stat counts via the unauthenticated /api/public-stats route
  // (Admin SDK server-side — see lib/public-stats-data.js) — never
  // hardcoded marketing numbers.
  useEffect(() => {
    loadPublicStats().then(setLiveStats);
  }, []);

  const heroStats = [
    { label: "Students Trained", value: liveStats?.students ?? 0, suffix: "+" },
    { label: "Expert Teachers", value: liveStats?.teachers ?? 0, suffix: "+" },
    { label: "Courses Offered", value: liveStats?.courses ?? 0, suffix: "+" },
    { label: "Total Enrollments", value: liveStats?.enrollments ?? 0, suffix: "+" },
  ];
    ...
    <>
      ...
      <nav className="sticky top-0 z-50 border-b border-white/10 bg-[#0B0D10]/95 shadow-sm backdrop-blur-md">
        <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-5 md:px-10">
          ...
          >
            <a href="#home" className="block py-2 text-sm font-medium text-white/70 transition hover:text-white">
              Home
            </a>
            <a href="#training" className="block py-2 text-sm font-medium text-white/70 transition hover:text-white">
              Training
            </a>
            <a href="#events" className="block py-2 text-sm font-medium text-white/70 transition hover:text-white">
              Events
            </a>
            <a href="#about" className="block py-2 text-sm font-medium text-white/70 transition hover:text-white">
              About
            </a>
            <a href="#contact" className="block py-2 text-sm font-medium text-white/70 transition hover:text-white">
              Contact
            </a>
          </div>
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            {verifiedUser ? (
              ...
              <div className="flex items-center gap-3">
                ...
                <Link href="/register" className="hidden text-xs font-semibold text-white/70 hover:text-white sm:block">
                  Register
                </Link>
              </div>
            )}
        ...
        {menuOpen && !verifiedUser && (
          <div className="border-t border-white/10 bg-[#0B0D10] px-5 pb-5 md:hidden">
            <Link href="/register" className="block py-3 text-sm font-semibold text-white">
              Register
            </Link>
          </div>
        )}
      ...
      <section id="home" className="bg-[#0B0D10]">
        <div className="mx-auto grid max-w-7xl items-center gap-12 px-5 py-14 md:grid-cols-2 md:gap-16 md:px-10 md:py-24">
          <div>
            <Eyebrow light>Next Academy · Learning Platform</Eyebrow>

            <h1 className="mt-5 max-w-xl text-5xl font-black leading-[1.05] tracking-[-.03em] text-white md:text-6xl lg:text-7xl">
              Learn today.
              <br />
              Grow your career.
              <br />
            ...
            <p className="mt-6 max-w-md text-base leading-7 text-white/60 md:text-lg">
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              <a
                href="#training"
                className="rounded-full bg-white px-6 py-3.5 text-sm font-bold text-[#111827] transition hover:bg-white/90"
              >
                Explore programs <span className="ml-3">→</span>
              </a>
              <button
                onClick={goToLearning}
                className="rounded-full border border-white/20 px-6 py-3.5 text-sm font-bold text-white transition hover:border-white/40"
              >
          ...
          <div className="flex justify-center md:justify-end">
            <PeopleDotCloud className="h-auto w-full max-w-[300px] sm:max-w-[380px]" />
          </div>
        </div>
      </section>
          ...
          <div className="mt-8 grid gap-4 md:grid-cols-4">
            {HOW_TO_HELP.map((item, index) => {
              ...
              return (
                ...
                >
                  <span className="text-2xl font-black text-[#E53935]">{item.number}</span>
                  <h3 className="mt-3 text-base font-bold text-[#111827]">{item.title}</h3>
      ...
      <section className="bg-[#0B0D10] py-14 md:py-16">
        <div className="mx-auto max-w-7xl px-5 md:px-10">
          <Eyebrow light className="justify-center">Why it matters</Eyebrow>
          <div className="mt-6">
      ...
      {/* GRADIENT CTA BANNER */}
      <section className="mx-auto max-w-7xl px-5 py-12 md:px-10 md:py-16">
        <div className="rounded-3xl bg-gradient-to-br from-[#E53935] to-[#F59E0B] px-6 py-14 text-center text-white md:px-16 md:py-16">
          <h2 className="mx-auto max-w-2xl text-3xl font-black leading-tight md:text-4xl">
            Your next skill is one enrollment away.
          </h2>
          <p className="mx-auto mt-4 max-w-lg text-sm leading-6 text-white/90 md:text-base">
      ...
      <footer className="border-t border-white/10 bg-[#0B0D10] text-white">
        <div className="mx-auto grid max-w-7xl gap-8 px-5 py-10 md:grid-cols-[1.4fr_1fr_1fr] md:gap-6 md:px-10 md:py-12">
          <div>
            <Brand light />
        ...
        <div className="border-t border-white/10 px-5 py-4 md:px-10">
          <div className="mx-auto flex max-w-7xl flex-col items-center justify-between gap-2 text-[11px] text-white/40 sm:flex-row">
            ...
            <span className="flex items-center gap-1.5">
              <span className="h-1.5 w-1.5 rounded-full bg-[#F04438]" /> Singapore
            </span>
          </div>
        </div>
      </footer>
