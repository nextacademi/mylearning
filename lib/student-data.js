...
async function loadCourseInfo(courseId) {
  ...
  try {
    ...
    // One unreadable/deleted course must never take down the whole
    // enrollments list — the caller already falls back to "Untitled
    // training" for a null entry, same as a genuinely missing course.
    console.error("[student-data] loadCourseInfo failed", { courseId, code: error?.code || "unknown" });
    return null;
  }
}
...
// A student's own manual (non-enrollment) invoices — Registration/
// Material/Exam Fee etc. Course-fee "invoices" are the enrollments
// themselves (see subscribeMyEnrollments above); this is only the other
// kind, read directly against `invoices` where firestore.rules already
// scopes to `resource.data.studentId == request.auth.uid`, same pattern
// as subscribeMyPayments.
export function subscribeMyInvoices(uid, onData, onError) {
  if (!db || !uid) return () => {};
  return onSnapshot(
    ...
    (error) =>
      reportQueryError(error, {
        ...
        queryDescription: `collection(invoices) where studentId == ${uid}`,
        uid,
        onError,
      }),
  );
...
// Perceived-speed only — mirrors training-service.js's prefetchTraining,
// but for Students (whose "My Training" tab is MyPaymentSummary, a
// listener-based page, not a one-shot fetch that a plain promise cache
// can dedupe). Fires the same two subscriptions early (dashboard mount,
// before the user has clicked into the tab) and keeps the latest emitted
// rows here; MyPaymentSummary seeds its own state from this cache on
// mount (instant if the prefetch already delivered data) and then
// establishes its own live subscription as before — a small duplicate
// listener while both are alive, not a replacement for it.
const enrollmentsCache = new Map();
const paymentsCache = new Map();
let warmedUid = null;
let stopWarming = null;

export function prefetchMyTraining(uid) {
  if (!uid || warmedUid === uid) return;
  if (stopWarming) stopWarming();
  warmedUid = uid;
  const unsubEnrollments = subscribeMyEnrollments(uid, (rows) => enrollmentsCache.set(uid, rows), () => {});
  const unsubPayments = subscribeMyPayments(uid, (rows) => paymentsCache.set(uid, rows), () => {});
  stopWarming = () => {
    unsubEnrollments();
    unsubPayments();
  };
}
export const getCachedEnrollments = (uid) => enrollmentsCache.get(uid);
export const getCachedPayments = (uid) => paymentsCache.get(uid) || [];
