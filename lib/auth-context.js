...
async function createProfile(user, role = "Student") {
  ...
  if (!existing.exists()) {
    // Every self-registered account starts as "Student" — active
    // immediately, no approval gate, no dashboard blocked.
    const status = "active";
...
export function AuthProvider({ children }) {
  ...
  useEffect(() => {
    ...
    // Completes a social sign-in that started with signInWithRedirect and
    // surfaces its error; the profile itself is created by the
    // onAuthStateChanged handler below.
  ...
  const actions = useMemo(
    () => ({
      ...
      async socialLogin(providerName) {
        ...
        try {
          // Full-page redirect on the production domain (the small Firebase
          // popup renders Google's consent page cramped and scrolling);
          // other hosts keep the popup since the redirect flow needs the
          // auth handler to share the site's domain.
          if (/(^|\.)nextacademy\.sg$/.test(window.location.hostname)) {
